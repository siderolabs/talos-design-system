...
2. **Emit the tokens** the way the README describes for the product's stack: `tokens.css` plus `tailwind.css` for Tailwind, the `tokens.scss` mixins for Sass and Bootstrap, `tokens.css` plus `type.css` for anything else. Emit the light theme under whatever selector the product already uses for it.
...
7. **Replace colour literals** with semantic roles, or delete them where the theme now supplies the colour. Colour and the pastel pill mean status. Anything that is not a status gets neither.
