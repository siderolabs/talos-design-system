...
This is the style guide for every Talos product interface: Omni, Talos Director, Xenia, and the Sidero portal. It carries the values, the rules for using them, and the enforcement. If you are making a UI suggestion (including with an AI tool), point it at this document and the token package rather than at a screenshot of an existing product.

The values come from the Talos Director skin, which is a faithful implementation of the Sidero brand visual guide. Director is the reference for how Talos software looks. Where this guide departs from what Director ships today, it says so and says why.
...
It does not cover components. Omni is Vue with Tailwind, Director is React with Bootstrap, and a shared component library would mean rewriting one of them. Consistency at the component level comes from the anatomy notes here, implemented natively in each product.
...
`compact` is new relative to the scale Omni shipped in August. Director's density is a feature, Neil is explicit about not wasting screen real estate, and a menu that jumps 12 to 24 forces dense tables into the wrong step.
...
Both faces ship with the token package as woff2 and are served by the product, never fetched from Google at runtime. A product that loads its type from a CDN loses it in an air-gapped install, and loses it quietly: the page falls back to a system font and still works, so the report never comes. Director does this today.
...
**Tab budget: ten.** Past ten, group them. Fifteen ungrouped tabs is a second navigation system with no hierarchy, and it wraps to two rows at 1024px, which Director's VM detail page does today. Group by concern (Protection holding snapshots and backups, Observability holding metrics and alerts and events and logs) or move to a two-level detail layout.
...
**The neutral ramp is re-pointed, not just the accent.** Mintlify derives its greys from the primary colour, which is how the docs site ended up with a pink-tinted grey. Changing the accent alone leaves the chrome cold. The generated stylesheet maps `--gray-50` through `--gray-950` onto the cream and ink ramps, which is what actually makes the page read as the same product as Omni and Director.

## Deliberate deviations from what Director ships today

Three, all for contrast, all verified by `npm run contrast`.

1. **The text ramp shifts up.** Director's dim `#8A8A88` and faint `#5A5A58` are below AA as body text on every surface in the ladder (2.2:1 to 2.9:1). The four visible steps move lighter in dark mode and darker in light mode, and `#5A5A58` becomes disabled-only.
...
Director's own codebase already carries a global minimum contrast ratio of 1.55 with a scoped remediation for green fills. That remediation is good work and its conclusion is the one adopted here: solid status fills carry dark labels. These three changes extend the same logic to the cases it did not reach.
