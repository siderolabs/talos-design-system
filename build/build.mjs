...
function buildCss() {
  ...
  lines.push(' * use (Director: html[data-bs-theme="light"], via the SCSS mixin).')
  lines.push(' */')
...
function buildTailwind() {
  const lines = [
    ...
    ' *',
    ' * Import tokens.css first, then this file. `@theme inline` is deliberate:',
    ' * it makes the generated utilities reference the custom property rather',
    ' * than copy its value, which is what lets the theme switch at runtime.',
    ' */',
    '',
...
function buildMintlify() {
  ...
  const lines = [
    ...
    ' * is vendored rather than imported: copy it to public/style.css in',
    ' * siderolabs/docs and re-copy it on a version bump.',
    ' *',
  ...
  lines.push(
    '/* ------------------------------------------------------------- site rules */',
    '',
    '/* Carried over from the docs repo. Colours now come from tokens; the layout',
    '   fixes are docs-specific and stay here. */',
    '',
    '.nav-logo {',
    ...
    '  width: 100%;',
    `  margin: var(${PREFIX}space-compact) 0;`,
    '',
    '  th,',
    '  td {',
    '    text-align: left;',
    ...
    '',
    '  td {',
    `    border: var(${PREFIX}border-hairline) solid var(${PREFIX}border-default);`,
    '  }',
    '',
    '  tbody {',
    `    border: var(${PREFIX}border-hairline) solid var(${PREFIX}border-default);`,
    ...
    '    tr {',
    '      &:nth-child(even) {',
    `        background-color: var(${PREFIX}surface-subtle);`,
    '      }',
    '',
    '      &:hover {',
    `        background-color: var(${PREFIX}surface-hover);`,
    '      }',
    '    }',
    '  }',
    '}',
    ...
    '.prose table th,',
    '.prose table td {',
    `  padding: var(${PREFIX}space-tight) var(${PREFIX}space-compact) var(${PREFIX}space-tight) var(${PREFIX}space-base);`,
    '}',
    '',
